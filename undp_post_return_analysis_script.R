...
clean_dataset_hh <- read_excel("input/IMPACT_dataset_returnees_UNDP.xlsx", guess_max = 5000) %>% fix_data_type() #%>% select(-)
...
analyze_data_hh <- function(cols_to_analyze, compare_by = NULL)
{
  ...
  new_sheet_name <- str_c(if_else(is.null(compare_by), "All", str_c(compare_by, sep='_')[1]), new_sheet_number, sep = " ")
